...
CLala::CLala() : m_bIsGot(false) , m_dwShootDelay(GetTickCount())
{
}

CLala::CLala(float fX, float fY) :m_bIsGot(false)
{
	m_tInfo.fX = fX;
	m_tInfo.fY = fY;
}
...
void CLala::Initialize()
{
	...
	GET(CResourceMgr)->Insert_Png(L"../Resources/Images/Item/Weapon/Lala.png", L"Lala");
	GET(CResourceMgr)->Insert_Bmp(L"../Resources/Images/Item/Weapon/Lala.bmp", L"LalaBmp");
	m_wsFrameKey = L"LalaBmp";
...
void CLala::Render(HDC hDC)
{
	...
	{
		HDC hMemDC = GET(CResourceMgr)->Find_Bmp(L"LalaBmp");
		GdiTransparentBlt(
			hDC,
	...
	{
		// 1. 필요한 값 및 객체 준비
		Bitmap* _bmp = (Bitmap*)GET(CResourceMgr)->Find_Png(L"Lala");
		if (!_bmp) return;
