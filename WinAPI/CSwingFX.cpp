...
void CSwingFX::Initialize()
{
	...
	GET(CResourceMgr)->Insert_Png(L"../Resources/Images/Item/Effect/SwingFX.png", L"SwingFX");
	m_wsFrameKey = L"SwingFX";
	m_iFrameWidth = 147.f;
	m_iFrameHeight = 160.f;
...
void CSwingFX::Render(HDC hDC)
{
	...
	if (dynamic_cast<CPlayer*>(GET(CPlayerMgr)->GetPlayer())->Get_IsAttack())
	{
		...
		// 1. 필요한 값 및 객체 준비
		Bitmap* _bmp = (Bitmap*)GET(CResourceMgr)->Find_Png(L"SwingFX");
		if (!_bmp) return;
